...
public:
  TransformClassTMS(int levelRange_max, int levelRange_min) {
  ...
  int latToPixelY(double latitude, int level) {
    ...
    int pixelY = 255 - (int)floor(y * this->_getMapSize(level) * 256) % 256;
  ...
  double pixelYToLat(int pixelY, int tileY, int level) {
    ...
    double latitude = atan(sinh(M_PI * (-1 + 2 * (tileY + 1 - pixelYToTileAddition) / this->_getMapSize(level)))) * 180.0 / M_PI;
